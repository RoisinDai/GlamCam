...
def process_frame(unity_joints_frame, unity_clothes_frame, kinect_joints_frame, live_human_frame) -> np.ndarray:
    ...
    unity_coords = UnityUtils.get_joints_coord(UnityUtils.segment_joints(unity_joints_frame))
    kinect_coords = KinectUtils.get_joints_coord(KinectUtils.segment_joints(kinect_joints_frame))

    # Segment the clothing from the Unity frame
    cloth_transparent = UnityUtils.apply_mask_to_image(
        unity_clothes_frame,
        UnityUtils.segment_clothes(unity_clothes_frame)
    )
