...
public class AvatarController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        // // Proceed to manually set rotation
        // Transform leftForearm = animator.GetBoneTransform(HumanBodyBones.LeftLowerArm);

        // if (leftForearm == null)
        // {
        //     Debug.LogError("Left Forearm bone not found in Humanoid rig.");
        //     return;
        // }

        // Quaternion targetRotation = Quaternion.Euler(0, 180, 0); // Rotate forearm upwards
        // leftForearm.localRotation = targetRotation;

        // Debug.Log($"Left Forearm Rotation Applied: {leftForearm.localRotation.eulerAngles}");
    ...
    private void ApplyForwardKinematics(Kinect.Body body)
    {
        ...
        foreach (var (Start, End, UnityBone) in boneMapping)
        {
            ...
            // boneDirection = new Vector3(-boneDirection.z, boneDirection.y, boneDirection.x); // Swap axes as necessary
            boneDirection = new Vector3(boneDirection.x, boneDirection.y, -boneDirection.z);

            // Define T-pose offset if needed
            Quaternion tPoseOffset = Quaternion.Euler(-90, 0, 0); // Adjustments specific to Mixamo rig
